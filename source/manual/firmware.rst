...
In the firmware section you can keep your PFWsense up to date and have the ability to install additional software.
...
Plugins are additional software packages that are available for PFWsense, usually they come with their own
...
.. Note::
    ...
    Although PFWsense does not support the configuration of a proxy server, for services like the firmware updater it is possible
...
.. Note::
    ...
    Upgrading to PFWsense BE is only possible when the installed community version number is lower than the
    last available business edition. E.g. you can upgrade **22.7.x** to **22.10.x**, but you can not upgrade
    **23.1** to **22.10**. You can always re-install using the installer found on the `business mirror <https://pfwsense-update.deciso.com/>`__
