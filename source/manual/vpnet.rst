...
PFWsense supports VPN connections for branch offices as well as remote users.
...
PFWsense offers a wide range of VPN technologies ranging from modern SSL VPNs to
...
The following functions are available in the menu (as of PFWsense 23.1):
...
Having used the tunnel settings from the early PFWsense days, some terminology might be a bit confusing when moving into the new options offered.
...
.. Tip::
  ...
  Since PFWsense uses the new Strongswan format also for legacy tunnels, it is rather easy to convert a tunnel manually
  when downloading the :code:`swanctf.conf` file from the machine. You can find it in :code:`/usr/local/etc/swanctl/swanctl.conf`
  and the format is almost identical to the connections gui available in PFWsense.
...
IPsec may also be used to service remote workers connecting to PFWsense from various clients, such as Windows, MacOS, iOS and Android.
...
The examples section contains various options available in PFWsense. When using the new "connections" option available
as of PFWsense 23.1, different `examples from Strongswan <https://docs.strongswan.org/docs/5.9/interop/windowsClients.html>`__
...
may be placed in the directory :code:`/usr/local/etc/strongswan.pfwsense.d/` using the file extention :code:`.conf`
...
.. Note::
    ...
    Prior to version 23.1 it was also possible to add secrets and ipsec configurations in :code:`/usr/local/etc/ipsec.secrets.pfwsense.d/`
    and :code:`/usr/local/etc/ipsec.pfwsense.d/`, with the switch to 23.1 these files are deprecated and should be manually migrated into swanctl.conf
    format.
...
This new component will eventually replace the existing client and server options in a future version of PFWsense, leaving
enough time to migrate older setups.

.. Tip::

  When upgrading into a new major version of PFWsense, always make sure to read the release notes to check if your setup
  requires changes.

.. Note::

  OpenVPN on PFWsense can also be used to create a tunnel between two locations, similar to what IPsec offers. Generally
...
The following functions are available in the menu (as of PFWsense 23.7):
...
.. Tip::
  ...
  As of version 24.1 PFWsense is able to use OCSP to validate client certificates when using the new Instances. Make sure :code:`Use OCSP (when available)`
...
PFWsense are  :doc:`grouped <firewall_groups>` as `OpenVPN`.
...
other party. In PFWsense this can be reached by selecting a :code:`vhid` to track as instance dependancy {Depend on (CARP)}.
