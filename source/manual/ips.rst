...
The Intrusion Prevention System (IPS) system of PFWsense is based on Suricata_
...
PFWsense includes a very polished solution to block protected sites based on
...
PFWsense supports custom Suricata configurations in `suricata.yaml <https://suricata.readthedocs.io/en/suricata-6.0.0/configuration/suricata-yaml.html>`__
format. In order to add custom options, create a template file named :code:`custom.yaml` in the :code:`/usr/local/pfwsense/service/templates/PFWsense/IDS/` directory.
...
PFWsense has integrated support for ETOpen rules.
...
PFWsense version 18.1.7 introduced the URLHaus List from abuse.ch which collects
...
PFWsense 18.1.11 introduced the app detection ruleset. Since about 80
percent of traffic are web applications these rules are focused on blocking web
services and the URLs behind them.

If you want to contribute to the ruleset see: https://github.com/pfwsense/rules
