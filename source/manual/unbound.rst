...
Since PFWsense 17.7 it has been our standard DNS service, which on a new install is enabled by default.
...
``/usr/local/etc/unbound.pfwsense.d`` directory. These files will be automatically included by
...
.. Note::
  ...
  To get the same effect as placing the file in the sample above directly in ``/usr/local/etc/unbound.pfwsense.d`` follow these steps:

  #. Create a ``+TARGETS`` file in ``/usr/local/pfwsense/service/templates/sampleuser/Unbound``::

      sampleuser_additional_options.conf:/usr/local/etc/unbound.pfwsense.d/sampleuser_additional_options.conf
  ...
  #. Check the output in the target directory::
      ...
      # show generated file
      cat /usr/local/etc/unbound.pfwsense.d/sampleuser_additional_options.conf
