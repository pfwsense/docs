...
PFWsense can use an LDAP server for authentication purposes and for authorization
...
Your PFWsense firewall needs to be fully configured and able to access the LDAP server.
...
.. Tip::
   ...
   **&(memberOf=CN=myGroup,CN=Users,DC=pfwsense,DC=local)** to select only members
...
.. Note::
    ...
    Groups will be extracted from the first :code:`CN=` section and will only be considered when already existing in PFWsense.
...
.. Note::
    ...
    (not available in the community version of PFWsense)
