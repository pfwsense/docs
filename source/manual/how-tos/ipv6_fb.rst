...
DSL, Cable and Fiber in Germany. This guide will setup a PFWSense
behind a FB, handover delegated prefixes from the provider and
configure local interfaces on the PFWSense to cope with dynamically changing IPv6 prefixes.
...
The PFWsense has an interface pointing to the ISP named `WAN` and has three internal 
...
all four segments on the PFWsense. 
...
the PFWsense host. Select :menuselection:`Internet --> Permit Access --> <your OPN Host> --> IPv6 Settings --> Open firewall for delegated IPv6 prefixes of this device`
...
On the PFWSense go to :menuselection:`Interfaces --> WAN` and set the configuration type for IPv6 to **DHCPv6**. On the bottom part of the dialog in
...
2. the setting **Request only an IPv6 prefix** is the important part. 
   ...
   the PFWsense as a router and really delegates a prefix. The PFWSense will only get a link-local `0xfe80`
   address but that is fine. If this checkbox is not selected the FB considers the PFWsense as an end-user device
   and plainly refuses to delegate a prefix to it. The PFWsense end up with an valid IPv6 address but with `/64`
...
DNS options                  empty        this gives away the PFWsense as DNS server with the current dynamic IP
===========================  ===========  ======================================================================
...
PFWSense will automatically prefix this pattern with the dynamically acquired prefix.
...
While discovering the specifics of IPv6 behind a FB in combination with PFWsense the first point of debugging was always
connecting via SSH to PFWsense on the CLI. 
