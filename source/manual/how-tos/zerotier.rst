...
networks. Adding a Network here will join your PFWsense installation to the
...
Adding a network does **not** automatically cause your PFWsense installation
to join that network. In order to join that network it **must** be ``Enabled``
first. Likewise, to remove your PFWsense node from the configured network
...
join the network on the portal. Simply enabling the network on your PFWsense
...
can found on the PFWsense Zerotier Overview menu item, under the Information
...
This tab shows each configured **and** enabled network that this PFWsense
...
now wish to assign the Zerotier virtual interface on PFWsense in order to
avail of PFWsense functionality such as firewalling and routing (using OSPF
for example).
...
.. WARNING::
    ...
    happen shortly afterwards, hence locking the interface tells PFWsense not
