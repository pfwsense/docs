...
Besides the web frontend, SSH and a locally connected monitor (if your device supports it), PFWsense can also be
controlled via serial. Accessing PFWsense via serial is similar to accessing via SSH, but unlike SSH, the system can
be accessed at any time, even when PFWsense is not accessible over the network. This makes it especially useful for installing PFWsense,
...
* PFWsense installation must provide a serial interface (virtual or hardware)
...
If you already installed PFWsense via a non-serial installer, serial access needs to be turned on. To do this, open
the web interface, navigate to :menuselection:`System --> Settings --> Administration`, scroll down to 'Console' and set the primary or
secondary console to 'Serial console'. Note: this is **only** necessary if you already installed PFWsense, and did not
...
If PFWsense is running, you will now be asked for your username and password if authentication is enabled. Otherwise
