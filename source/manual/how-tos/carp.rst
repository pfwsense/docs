...
One of the more powerful features of PFWsense is to set-up a redundant
...
PFWsense includes a mechanism to keep the configuration of the backup
...
+-------------------------+------------------------------------+
| Virtual password        | pfwsense (the example uses this)   |
+-------------------------+------------------------------------+
| VHID Group              | 1                                  |
+-------------------------+------------------------------------+
...
default NAT configuration is for PFWsense is to use Automatic outbound NAT 
...
each others pools. If DNS requests are also forwarded by PFWsense, make
...
First go to :menuselection:`System --> High availability --> Status` in the PFWsense webinterface and check if
...
connected you should be able to look at the state table on both PFWsense
