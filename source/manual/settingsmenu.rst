...
Besides the configuration options that every component has, PFWsense also contains a lot of general settings
...
The settings on this page concerns logging into PFWsense. The “Secure Shell” settings are described under
...
DNS Rebind Check                               PFWsense contains protection against
...
In order to access PFWsense via SSH, SSH access will need to be configured via :menuselection:`System --> Settings --> Administration`.
...
============================================== ========================================================================
Use the virtual terminal driver (vt)           When unchecked, PFWsense will use the older sc driver.                |
...
+---------------------------------+------------------------------------------------------------------------------------+
| Do not use the local DNS        | When enabling local DNS services such as Dnsmasq and Unbound, PFWsense will use    |
...
A list of possible values can be obtained by issuing ``sysctl -a`` on an PFWsense shell.
...
As of PFWsense 20.7 we changed our default logging method to regular files.
...
With PFWsense version 19.7, syslog-ng for remote logging was introduced.
...
.. Note::
    ...
    errors are quite common in these type of setups. On PFWsense the general system log usually contains more details.
    ...
    syslog in PFWsense (using the gui).
