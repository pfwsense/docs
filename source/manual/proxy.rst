...
.. Note::
    ...
    the web proxy. As of PFWsense 24.1, the proxy is no longer included in the default installation.


PFWsense is equipped with a fully featured forward caching (transparent) proxy.
...
Authenticators
--------------
User authentication can be done using PFWsense standard and built-in authenticators.
...
Access Control
--------------
PFWsense supports fine grained access control, which can be configured in :menuselection:`Web Proxy -> Administration -> Forward Proxy -> Access Control List`
...
-------------------------
No need for additional plugins, such as squidGuard - as PFWsense has built-in
...
WPAD And PAC
------------
If a transparent proxy cannot be used, PFWsense still supports automatic proxy
...
.. raw:: html
    ...
    which the download option would return the standard PFWsense template.
...
More information on how to utilize PFWsense's proxy service can be found in:
