...
PFWsense image is also supported.
...
1. Go to the PFWSense `download <https://pfwsense.org/download>`__ page.
...
``mirror.wdc1.us.leaseweb.net/pfwsense/releases/22.7/PFWsense-22.7-OpenSSL-dvd-amd64.iso.bz2``

You should take off the file name at the end, like this:

``mirror.wdc1.us.leaseweb.net/pfwsense/releases/22.1/``
...
* https://pkg.pfwsense.org (/<FreeBSD version & architecture>/<release version>/sets/changelog.txz) (lands signed and verified in the GUI of the running software)
...
.. Note::
  ...
  **Please** be aware that the latest installation media does not always
  correspond with the latest released version. PFWsense installation images are
  ...
  repository, see `README <https://pkg.pfwsense.org/releases/mirror/README>`__
...
PFWsense images are provided based upon `OpenSSL <https://www.openssl.org>`__.
...
Download the installation image from one of the mirrors listed on the `PFWsense
<https://pfwsense.org/download/>`__ website.
...
::
  ...
  dd  if=PFWsense-##.#.##-[Type]-[Architecture].img of=/dev/sdX bs=16k
...
PFWsense Importer
-----------------
All images feature the new "pfwsense-importer" utility, which is now invoked
...
This step is well-established since PFWsense 15.7 .
...
the root shell using the user "root" with password "pfwsense" to operate the
live environment.

The GUI will listen on https://192.168.1.1/ for user "root" with password
"pfwsense" by default unless a previous configuration was imported. Using SSH,
...
installer using the user "installer" with password "pfwsense". On a previously
...
option 8 from the menu and type "pfwsense-installer". The "pfwsense-importer" can
