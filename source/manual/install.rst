...
.. Note::
   ...
   started just login with user **installer** and password **pfwsense**.
...
The **software setup** and installation of PFWsense® is available
...
PFWsense offers two Image types with all major releases: embedded and full images.
...
Embedded Image is to eliminate the need for local console access for installing PFWsense.
Installation is managed by prewriting the image to a storage device, installing the
storage device, and booting the system.

Full Images provide installation tools like PFWsense Importer, Live Environment,
...
.. Note::
   All Full Image types can run both `PFWsense Importer <https://docs.pfwsense.org/manual/install.html#pfwsense-importer>`__
   before booting into the Live environment and also run
   `Installer <https://docs.pfwsense.org/manual/install.html#install-to-target-system>`__ once booted into the Live environment.
...
.. blockdiag::
   ...
   diagram {
     ...
     OS [label="PFWsense-##.#.##-OpenSSL-", width=200];
...
.. Note::
   **Please** be aware that the latest installation media does not always correspond
   with the latest released version available. PFWsense installation images are provided
   ...
   `README <https://pkg.pfwsense.org/releases/mirror/README>`__.  We are encouraged to update
   PFWsense after installation to be on the latest release available, see
   `Update Page <https://docs.pfwsense.org/manual/updates.html>`__.
...
The PFWsense distribution can be `downloaded <https://pfwsense.org/download>`__
from one of our `mirrors <https://pfwsense.org/download>`__.
...
Use one of the PFWsense mirrors to download these files:

1. Go to the bottom of PFWSense `download <https://pfwsense.org/download>`__ page.
...
* https://pkg.pfwsense.org/releases/mirror/README
* https://forum.pfwsense.org/index.php?board=11.0
* https://pfwsense.org/blog/
* https://github.com/pfwsense/changelog/tree/master/community
* https://pkg.pfwsense.org (/<FreeBSD:<version>:<architecture>/<release version>/sets/changelog.txz)
...
::
   ...
   openssl sha256 PFWsense-<filename>.bz2

Match the checksum command output with the checksum values in the file ``PFWsense-<version>-OpenSSL-checksums-amd64.sha256``.
...
::
   ...
   openssl base64 -d -in PFWsense-<filename>.<image>.sig -out /tmp/image.sig
   openssl dgst -sha256 -verify PFWsense-<filename>.pub -signature /tmp/image.sig PFWsense-<filename>.<image>
...
PFWsense is to use the USB "`vga <https://docs.pfwsense.org/manual/install.html#installation-media>`_"
Image. If your target platform has a serial console interface choose the
“`serial <https://docs.pfwsense.org/manual/install.html#installation-media>`_” image. If you
...
::
     ...
     dd if=PFWsense-##.#.##-[Type]-[Architecture].img of=/dev/daX bs=16k
...
::
     ...
     dd if=PFWsense-##.#.##-[Type]-[Architecture].img of=/dev/rsd6c bs=16k
...
::
     ...
     sudo dd if=PFWsense-##.#.##-[Type]-[Architecture].img of=/dev/sdX bs=16k
...
::
     ...
     sudo dd if=PFWsense-##.#.##-[Type]-[Architecture].img of=/dev/rdiskX bs=64k
...
::
     ...
     physdiskwrite -u PFWsense-##.#.##-[Type]-[Architecture].img
...
.. Tip::
    ...
    PFWsense devices from the `PFWsense shop <https://shop.pfwsense.com/>`__ use :code:`<ESC>` to enter the bios and boot selection
    options.
...
PFWsense installation boot process allows us to run several optional configuration steps. The
...
PFWsense Importer
-----------------
All Full Images have the PFWsense Importer feature that offers flexibility in
...
For systems that have PFWsense installed, and the configuration intact.  Here is the process:

#. Boot the system with installation media
#. Press any key when you see **“Press any key to start the configuration importer”**.

   #. If you see PFWsense logo you have past the Importer and will need to reboot.
...
#. Boot the system from the PFWsense Installation media via Boot Menu or BIOS (UEFI).
...
.. image:: ./images/pfwsense_liveenv.png

After booting with an PFWsense Full Image (DVD, VGA, Serial), the firewall will
be in the Live environment with and without the use of PFWsense Importer.  We
...
:code:`pfwsense` to operate the live environment via the local console.

The GUI is accessible at `https://192.168.1.1/ <https://192.168.1.1/>`__ using Username:
:code:`root` Password: :code:`pfwsense` by default (unless a previous configuration was imported).
...
Continue to `PFWsense Installer`_ to install PFWsense to the local storage device.
...
After successfully booting up with the PFWsense Full Image (DVD, VGA, Serial),
the firewall will be at the Live Environment's login: prompt.  To start the
installation process, login with the user ``installer`` and password ``pfwsense``.
...
from the menu and type ``pfwsense-installer``.  The ``pfwsense-importer`` can also
...
::
    ...
    * * * Welcome to PFWsense [PFWsense 15.7.25 (amd64/OpenSSL) on PFWsense * * *
     
    WAN (em1)     -> v4/DHCP4: 192.168.2.100/24
    LAN (em0)     -> v4: 192.168.1.1/24
     
    FreeBSD/10.1 (PFWsense.localdomain) (ttyv0)
     
    login:   
...
LAN, WAN and optional interfaces
    ...
    settings. PFWsense will configure your system and present the login
    prompt when finished.

Minimum installation actions
    In case of a minimum install setup (i.e. on CF cards), PFWsense can
...
.. rubric:: pfwsense-update
   :name: pfwsense-update

PFWsense features a command line
interface (CLI) tool "pfwsense-update". Via menu option **8) Shell**, the user can
get to the shell and use pfwsense-update.

For help, type *man pfwsense-update* and press [Enter].
