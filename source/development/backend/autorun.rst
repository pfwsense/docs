...
PFWsense offers an easy method to plug in custom scripts during boot stages and assorted system events.
...
Part of the bootup process of PFWsense is probing the available rc(8) configuration files in */etc/rc.conf.d/*, when a daemon is enabled, the system will call the regular rc(8) start command.
...
::
    ...
    squid_enable=YES
    squid_pfwsense_bootup_run="/usr/local/pfwsense/scripts/proxy/setup.sh"
