...
Since PFWsense runs on a fork of FreeBSD, DTrace is natively available on the 
...
We will be looking at setting up DTrace on PFWsense, and running a sample script 
...
this document provides an explanation in combination with the use of PFWsense-specific tools. 
...
Make sure the PFWsense/src repository is present on the system and navigate to 
...
The pfwsense-tools require the name to be :code:`GENERIC`, so we will leave it as is. 
It is advisable to create a backup of the original :code:`GENERIC` file though.
Make sure the PFWsense-tools repository is installed and navigate to :code:`/usr/tools/config/[VERSION]`. 
...
Install the package using our :code:`pfwsense-update` command:
