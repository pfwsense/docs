...
.. Tip::
    ...
    When using devices from the PFWsense shop (https://shop.pfwsense.com), usually there's a mini-usb to serial cable included
...
Swith the power and wait for the PFWsense splash screen to appear, the choose "Boot [S]single User" which should be option 2 in the list.
(press 2)
...
.. code-block:: sh
    ...
    pfwsense-shell password
    reboot

The `pfwsense-shell` will ask for a confirmation, respond with `Y`
